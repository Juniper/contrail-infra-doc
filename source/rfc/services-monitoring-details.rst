...
* nl01-jnpr.opencontrail.org (10.84.35.178; talk to Zookeeper) - down
* nl02-jnpr.opencontrail.org (10.84.35.185; talk to Zookeeper)

**Nodepool builder**

* nb01-jnpr.opencontrail.org (10.84.35.154; behind VPN)

**Zuul scheduler, Zuul merger, Zuul web, Zookeeper, Apache2**
**(Zuul WebUI proxy)**
...
* Long living connection (Nodepool Launcher <--> Zookeeper) could be
...
Sometimes Nodepool Launcher disconnect from Zookeeper and test builds are not
...
per 10 min (most of the time not passing threshold of 20) to nearly hundred,
or even more, requests per 10 min (and staying at this). At the same time,
Zuul queued jobs\** and Zuul running jobs\*\** counters stay below 10.
...
**But we must keep in mind, that those stats should be considered simultaneously.**

If Nodepool nodes requests are high and Zuul Executor jobs are also high
it could indicate some kind of overload, but not a connectivity error.
...
Latest such error was investigated by Bartosz Pankowski (April 16th 2019)
and he stated that:

  zk and node pool has an error on exception during killing vm's basically got the wrong id and could not proceed that.
  I ve investigated and restarted nodepool-luncher

(https://contrail-cloud.slack.com/archives/G8RTV20H2/p1555450596029900)
...
**Try of correlate nodepool errors to Zookeeper logs:**
